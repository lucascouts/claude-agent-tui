...
test("fork sha: vendor-time verified the SHA equals the v0.57.0 tag (R1.1)", () => {
  const p = readProvenance();
  // The upstream `.git` is removed when the fork is vendored into the project
  // repo, so `git rev-parse v0.57.0` cannot be re-run in regression. The re-anchor
  // (story 074) performs that equality check against the live upstream `.git` at
  // vendor time and records the outcome as `tagShaVerified`. This pins the intent of
  // the original assertion (`git rev-parse v0.57.0` equals the captured SHA) durably.
