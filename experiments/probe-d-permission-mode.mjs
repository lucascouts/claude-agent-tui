...
async function runMode(mode, idx) {
  ...
  const cmd = `claude --session-id ${sid} ${flag} --settings "${SETTINGS}"`
    .replace(/\s+/g, " ")
    .trim();
  const offset = logSize();
  const p = pty.spawn("bash", ["-lc", cmd], {
    name: "xterm-256color",
    cols: 120,
    rows: 40,
    cwd: TMP,
  ...
  const r = {
    mode,
    flagPassed: flag || "(none — default)",
    hookFired: false,
    payloadPermissionMode: null,
    toolNames: [],
    fileCreated: false,
    tuiPrompted: null,
  };
  try {
    ...
    p.write("\x15");
    await delay(60);
    p.write(`crie um arquivo chamado ${file} com exatamente o texto: oi`);
    await delay(60);
    p.write("\r");
    ...
    for (let i = 0; i < 60; i++) {
      ...
      if (pm.length) {
        r.hookFired = true;
        r.payloadPermissionMode = pm[pm.length - 1];
        r.toolNames = [...new Set(toolNamesIn(fresh))];
        break;
      }
...
const modeMatches = results.every((r) =>
  r.mode === "default" ? true : r.payloadPermissionMode === r.mode,
);
let interpretation;
if (allFired && modeMatches)
  interpretation =
    "CONFIRMED: the PreToolUse hook fires in EVERY mode (incl. bypass) and the payload's permission_mode matches the spawned mode → the decider can branch on call.permissionMode to honor the selector.";
else if (allFired && !modeMatches)
  interpretation =
    "Hook fires in all modes but permission_mode in the payload does NOT match — branch on session.modes.currentModeId instead of the payload.";
else
  interpretation =
    "Hook did NOT fire in some mode — inspect per-mode hookFired (if bypass skips the hook, the fix differs).";

console.log(
  JSON.stringify(
    { probe: "D — permission-mode hook payload", hookLog: LOG, results, interpretation },
    null,
    2,
  ),
);
